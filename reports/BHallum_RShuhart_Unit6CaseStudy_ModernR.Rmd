...
## KNN Classification on all Macs
```{r}
train_Y <- factor(trainByAvgSig$posXY)
train_X <- select(trainByAvgSig, -posXY, -posX, -posY)
test_X <- select(testAllMacs, -posXY, -posX, -posY)

#k = 1
test_estk1 <- as.character(knn(train_X, test_X, train_Y, k=1))

#k = 3
test_estk3 <- as.character(knn(train_X, test_X, train_Y, k=3))

#k = 5
test_estk5 <- as.character(knn(train_X, test_X, train_Y, k=5))

```

```{r}
errCalc <- function(EstimateXY, TruthX_Y){
  tmp <- cbind(Est_posXY = EstimateXY, TruthX_Y) %>%
          separate(Est_posXY, c("Est_posX", "Est_posY"), sep="-") %>%
          select(Est_posX, Est_posY, posX, posY) %>%
          mutate(Est_posX = as.numeric(Est_posX),
                 Est_posY = as.numeric(Est_posY))
  #print(tmp)
  sum( rowSums( (tmp[,c("Est_posX", "Est_posY")] - tmp[,c("posX", "posY")])^2) )
}

errCalc(test_estk1, testAllMacs[,c("posX","posY")])
errCalc(test_estk3, testAllMacs[,c("posX","posY")])
errCalc(test_estk5, testAllMacs[,c("posX","posY")])
...
ggplot(k_compare, aes(k, MSE)) + geom_line()
